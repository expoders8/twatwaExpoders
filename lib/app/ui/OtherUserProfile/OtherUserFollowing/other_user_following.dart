...
class OtherUserFollowingPage extends StatefulWidget {
  final String? userId;
  const OtherUserFollowingPage({super.key, this.userId});

  @override
  State<OtherUserFollowingPage> createState() => _OtherUserFollowingPageState();
}

class _OtherUserFollowingPageState extends State<OtherUserFollowingPage> {
  final OtherUserFollowingController otherUserFollowingController =
      Get.put(OtherUserFollowingController());
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      otherUserFollowingController.selectedOtherUserId(widget.userId);
      otherUserFollowingController.fetchAllOtherfollowing();
      getUser();
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Center(
        child: Obx(
          () {
            if (otherUserFollowingController.isLoading.value) {
              return SingleChildScrollView(
                  physics: const NeverScrollableScrollPhysics(),
                  child: buildLazyloading());
            } else {
              if (otherUserFollowingController.followerList.isNotEmpty) {
                if (otherUserFollowingController
                    .followerList[0].data!.isEmpty) {
                  return Center(
                    child: SizedBox(
                      width: Get.width - 80,
                      child: const Text(
                  ...
                  return ListView.builder(
                    ...
                    itemCount: otherUserFollowingController
                        .followerList[0].data?.length,
                    itemBuilder: (context, index) {
                      var followerData =
                          otherUserFollowingController.followerList[0].data!;
